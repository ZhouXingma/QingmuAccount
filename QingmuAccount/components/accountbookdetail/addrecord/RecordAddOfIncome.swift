...
struct RecordAddOfIncome : View {
    ...
    @State var title:String = defaultAccountBookInRecordIcon[0].name
    @State var desc:String = ""
    @State var iconStr:String = defaultAccountBookInRecordIcon[0].iconStr
    ...
    // 图标
    @State var inRecordIcons:[AccountBookSetingOfIconModel] = defaultAccountBookInRecordIcon
    // 账本配置信息
    ...
    func loadAccountBookInfo() {
        if (nil != globalModel.accountBook) {
            do {
                ...
                if (nil != seting) {
                    inRecordIcons = seting!.inRecordIconSeting
                    if (inRecordIcons.count > 0) {
                        iconStr = inRecordIcons[0].iconStr
                        title = inRecordIcons[0].name
                    }
                }
            } catch {
                showLoadAccountBookInfoError()
